...
def delete_project(project_id: int, db: Session = Depends(get_db)):
    try:
        result = db.execute(text("DELETE FROM projects WHERE id = :id"), {"id": project_id})
...
def update_project(project_id: int, project: ProjectCreate, db: Session = Depends(get_db)):
    try:
        db.execute(text("""
            UPDATE projects SET 
                project_name = :name, idea = :idea, team_members = :team,
                roll_number = :roll, class_name = :class, year = :year,
                branch = :branch, sec = :sec, tools = :tools, technologies = :tech
            WHERE id = :id
        """), {
            "id": project_id,
            "name": project.project_name,
        ...
        if db.execute(text("SELECT ROW_COUNT()")).fetchone()[0] == 0:
            raise HTTPException(status_code=404, detail="Project not found")
        db.commit()
